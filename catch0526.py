...
fileHandler = logging.FileHandler('普通詐欺罪_112-6.log', mode='a', encoding='utf-8') # a: append, w: write  #建構方法
...
for i in range(112, 113): # 民國111~112
    month_day_list = [
        ...
        for page in page_url_list:
            ...
            for case_u in case_url_list:      
                try:
                    ...
                    logger.warning(f'id_id:{id_id}, case URL is invalid: {ve}')
                except Exception as e:
                    logger.warning(f'id_id:{id_id}, search {date_split}, case_number:{case_counter} need to fix or research: {e}')
except Exception as e:
    logger.warning(f'id_id:{id_id}, search {date_split}, case_number:{case_counter} need to fix or research: {e}')
finally:
    # 就寫成dataframe再轉成csv八
    tt = pd.DataFrame(all_case_list)
    tt.to_csv('./普通詐欺罪_112-6.csv', index=False)
    driver.quit()  # 確保程式結束後關閉 WebDriver
